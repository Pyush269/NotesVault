...
const NoteCard = ({ title, date, content, tags, isPinned, onEdit, onDelete, onPinNote }) => {
  return (
    <div className="border border-gray-200 rounded-xl p-4 bg-gradient-to-br from-[#fdfbfb] to-[#ebedee] 
                    hover:shadow-lg hover:scale-[1.01] transition-all ease-in-out flex flex-col justify-between h-full">
      ...
      <div className="flex flex-wrap items-center justify-between mt-4 gap-2">
        ...
        <div className="flex flex-wrap gap-2">
          {tags.map((item) => (
            <span
              key={item}
              className="px-2 py-1 text-xs rounded-full bg-blue-100 text-blue-700 font-medium hover:bg-blue-200"
            >
              #{item}
            </span>
          ))}
